...
// =========================
    ...
    public ResponseEntity<String> deleteMultipleExpenses(
        ...
        try {
            ...
            log.info("User {} deleting {} expenses", userId, expenseIds.size());

            expenseService.deleteMultipleExpenses(expenseIds, userId);

            return ResponseEntity.ok("Deleted " + expenseIds.size() + " expenses");
        } catch (Exception e) {
            log.error("Error deleting multiple expenses", e);
